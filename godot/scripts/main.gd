...
##   --screenshot=<pfad>  speichert nach 90 Frames ein Bild und beendet das Spiel.
...
func _process(_delta: float) -> void:
	...
	if _drive and _frames == 70:
		Input.action_release("move_forward")
		Input.action_release("move_right")
	if _frames == 90:
		var robot: Node3D = $Robot
		print("robot pos=%s yaw=%.1f deg" % [robot.global_position, rad_to_deg(robot.rotation.y)])
